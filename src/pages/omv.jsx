...
const OptimizedMultiplierView = () => {
    ...
    function multiplicationSteps(binMultiplicand, binMultiplier) {
        ...
        if (binMultiplier.length > max) {
            ...
            for (let index = binMultiplier.length; index < binMultiplicand.length; index++) {
        ...
        steps.push({ iteration: 0, multiplicand: binMultiplicand, product: product })
        let msbproduct = ''
        for (let index = 0; index < iteration; index++) {
            if (product[product.length - 1] == '1') {
                msbproduct = binaryFunctions.binaryAddition(binMultiplicand, product.substring(0, max))
                product = msbproduct + product.substring(max)
            }
            product = product.substring(0, product.length - 1)
            product = '0' + product
            // binMultiplier = binMultiplier.substring(0, binMultiplier.length - 1)
            // binMultiplier = '0' + binMultiplier
            steps.push({ iteration: (index + 1), multiplicand: binMultiplicand, product: product })
        }
        return steps
        // console.log(steps)
    }
    let multiply = (event) => {
        if (multiplicand == '' || multiplier == '') {
            ...
            let max = binMultiplicand.length
            // console.log('testing')
            // console.log(binMultiplier.length)
            // console.log(binMultiplicand.length)
            if (binMultiplier.length > max) {
                max = binMultiplier.length
            }
        ...
        <div className=' max-w-[1280px] flex items-center justify-between flex-wrap  mx-auto px-4 text-white m-9 mt-24'>
            <div className=' flex flex-col bg-[#2f2f2f]  rounded-3xl my-5 w-full'>
                <div className=' px-5 py-4'>
                    ...
                    <h1 className=' text-2xl font-bold text-center'>Binary Optimized Multiplication</h1>
                </div>
                <div className=' mb-1'>
                    <hr />
                </div>
...
export default OptimizedMultiplierView